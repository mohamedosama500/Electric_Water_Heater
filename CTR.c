...
void SW_Init(void){
 ...
 void SW_Update(void){
    ...
    tmp = ReadData;
                ...
                /*after the switch of decreases or increases the temp. is pressed we wait when its not pressed then
                 update temp. and make it to toggle the led 4 times by setting the while(o<4)
                 setting the time to be (5*10ms*100*2)=10 seconds*/
			...
			h++;/*the flag used for main while if no switch is pressed*/
